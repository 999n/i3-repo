#!/bin/bash
# Focus prev window in active group
...
# Prev index (wrap around), if focused not in group start from last
if [[ $FOCUSED_IDX -eq -1 ]]; then
    PREV_IDX=$(( TOTAL - 1 ))
else
    PREV_IDX=$(( (FOCUSED_IDX - 1 + TOTAL) % TOTAL ))
fi

PREV="${MARKS_ARRAY[$PREV_IDX]}"

# Get workspace of the target window and switch to it first, then focus
WS=$(i3-msg -t get_tree | jq -r --arg m "$PREV" '
...
i3-msg "[con_mark=\"$PREV\"] focus" > /dev/null 2>&1
TITLE=$(i3-msg -t get_tree | jq -r --arg m "$PREV" 'recurse(.nodes[]?, .floating_nodes[]?) | select(type == "object") | select(.marks[]? == $m) | .name' | head -1)
